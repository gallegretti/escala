...
export function EditorLeftMenu({ score, selectedTrackIndex, selectTrack, onNewTrack }: EditorLeftMenuProps) {
    const {
        closeDialog,
        isDialogOpen,
        openDialog
    ...
    return (
        <>
            <TrackDialog
                isOpen={isDialogOpen}
                onClose={closeDialog}
                onSave={(params) => { onNewTrack(params); closeDialog() }}></TrackDialog>
            <SideMenuDiv>
                {score?.tracks &&
                    <EditorTrackSelector
